<?php
/* $Id: functions.php,v 1.55.2.46 2004/10/22 12:14:07 ajv Exp $ */
/*
    XMB 1.9
...
* 
* @param    $any  takes a variable amount of strings, and tries to preload each template specified by one of these
...
* @param    $ignorespaces    bool, tells the function to replace any occurence of the sequence of characters to censor (on true), or to make sure it is used as a word before censoring it. This parameter defaults to false
...
function postify($message, $smileyoff='no', $bbcodeoff='no', $allowsmilies='yes', $allowhtml='yes', $allowbbcode='yes', $allowimgcode='yes', $ignorespaces=false, $ismood="no", $wrap="yes") {
    global $xmbuser, $table_posts, $tid, $imgdir, $bordercolor, $table_words, $table_forums, $table_smilies, $db, $smdir, $smiliecache, $censorcache, $smiliesnum, $wordsnum, $versionbuild, $lang, $fontsize;
    ...
    if ( $bballow) {
		...
		// Begin Hidden Text Hack

        $query= $db->query("SELECT count(author) FROM $table_posts WHERE tid='$tid' and author='$xmbuser'");
        $num = $db->result($query,0);
        if ($num==0) {
            $message = eregi_replace("\\[hide\\]([^\\[]*)\\[/hide\\]","<table align=\"center\" class=\"hidden\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"hidden\">$lang[hiddentext]</td></tr><tr><td class=\"hiddenmessage\">$lang[textdetect]</td></tr></table>", $message);
        }else{
            $message = eregi_replace("\\[hide\\]([^\\[]*)\\[/hide\\]","<table align=\"center\" class=\"hidden\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"hidden\">$lang[hiddentext]</td></tr><tr><td class=\"hiddenmessage\">\\1<br /><br /></td></tr></table>", $message);
        }

		// End Hidden Text Hack
