<?php
/* $Id: viewthread.php,v 1.33.2.29 2004/10/22 12:14:07 ajv Exp $ */
/*
    XMB 1.9
...
if (isset($_COOKIE['oldtopics'])) {
    put_cookie("oldtopics");            // remove non-functioning topics
}
...
$query = $db->query("SELECT tid, fid, subject, views, replies, closed, topped, pollopts FROM $table_threads WHERE tid='$tid'");
...
if (strlen($thread['subject']) > 126) {
    $thread['subject'] = substr($thread['subject'], 0, 127) . '...';
}
    ...
    while ($post = $db->fetch_array($querypost)) {
		...
		if($post['glowcolor'] == "") {
		  ...
		  // END SHOP HACK
			...
			if (!empty($post['theme']) && $post['theme'] != 0) {
            ...
            // BEGIN VICIOUS HACK
                if($SETTINGS['VH_status'] == "on"){
                    if($post['vicious'] != "0"){ $Fvhimg = (($post['vicious'] < 0) ? ($SETTINGS['VH_image1']) : ($SETTINGS['VH_image2'])); $VHpv = str_replace("-", "", $post['vicious']);
                        if($Fvhimg != "" && ($SETTINGS['VH_star'] <= $VHpv)){ $thaimage .= '<br />'; $Fi = ''; 
                            $theimage = floor($VHpv / $SETTINGS['VH_star']);
                            for($num = 0; $num < $theimage; $num++){ $Fi++;
                                if($Fi == ($SETTINGS['VH_addbr'] + 1)){ $thaimage .= '<br />'; $Fi='1'; }
                                $thaimage .= '<img alt="'.$SETTINGS[VH_title].': '.$post[vicious].'" src="'.$imgdir.'/'.$Fvhimg.'" />';
                            }
                        } 
                    }
                    $vicious = str_replace("*vicious*", $SETTINGS['VH_title'], str_replace("*rating*", $post['vicious'], $lang['VH_o04'])).''.$thaimage.'<br />
                    <a href="member.php?action=vicious&do=smite&member='.rawurlencode($post[username]).'&fr=view&tid='.$tid.'&page='.$page.'&pid='.$post[pid].'">'.$lang[VH_o01].'</a> '.$SETTINGS[VH_sep].' <a href="member.php?action=vicious&do=praise&member='.rawurlencode($post[username]).'&fr=view&tid='.$tid.'&page='.$page.'&pid='.$post[pid].'">'.$lang[VH_o02].'</a>';
                } else {
                    $vicious = '';
                } $post['vicious']=""; $thaimage="";
            // END VICIOUS HACK

        } elseif ($post['username'] == "") {
            $showtitle = $post['author'].'<br />';
            $post['author'] = $lang['VH_o09'];
            $vicious = "";
            $stars = '';
            $avatar = '';
            $post['postnum'] = 'N/A';
