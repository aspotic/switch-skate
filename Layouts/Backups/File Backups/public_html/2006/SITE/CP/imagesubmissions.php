...
<?php
	 ...
	 while ($row = mysql_fetch_array ($Result)) {
		   ...
		   if ($cattype == $row[category]) {
	 	   	    echo "$counter. <a href='?category=imagesubmissions&cattype=$cattype&image=$row[id]'>$row[title]</a> by $row[writer] [<a href='?category=viewimage&type=$type&remove=$row[id]'>Delete</a>] ";
		   		$counter++;
		   }
