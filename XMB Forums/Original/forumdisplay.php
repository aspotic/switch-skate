<?php
/* $Id: forumdisplay.php,v 1.15.2.29 2004/10/21 14:34:16 ajv Exp $ */
/*
    XMB 1.9.1
...
while($thread = $db->fetch_array($querytop)) {
	...
	if (isset($_COOKIE['oldtopics'])) {
		put_cookie("oldtopics");			// remove non-functioning hot topic cookie
	}

	$viewTopics = array();
	if ( isset($_COOKIE['viewtopics']) && $_COOKIE['viewtopics'] != '' ) {
		$viewTopics = explode($_COOKIE['viewtopics'], ",");
		$hotThread = in_array($thread['tid'], $viewTopics);
	} else {
		$hotThread = false;
	}

    if ( $thread['replies'] >= $hottopic && $lastvisit2 < $dalast && $hotThread  ) {
        $folder = "hot_red_folder.gif";
    } elseif ( $lastvisit2 < $dalast && $hotThread ) {
